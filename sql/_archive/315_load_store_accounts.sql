...
/*
Sprint: 3
Purpose: Load Store-based accounts so sales orders can resolve account_id
Source: Sales.Store (BusinessEntityID)
Target: tgt.account
*/
...
USING (
    SELECT
        ctl.fn_deterministic_guid('account', CAST(s.BusinessEntityID AS NVARCHAR(100))) AS account_id,
        s.Name AS account_name,
        'Organization' AS account_type,
        @batch_id AS migration_batch_id,
        'AdventureWorks2022' AS source_system,
        CAST(s.BusinessEntityID AS NVARCHAR(100)) AS source_primary_key,
